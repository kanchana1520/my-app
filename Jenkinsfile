node{
   ...
   sh 'docker build -t kanchanaravi1520/myweb:0.0.2 .'
   }
   stage('Docker Image Push'){
   withCredentials([string(credentialsId: 'dockerPass', variable: 'dockerPassword')]) {
   sh "docker login -u kanchanaravi1520 -p ${dockerPassword}"
    }
...
stage('Nexus Image Push'){
   ...
   sh 'docker run -d -p 8090:8080 --name tomcattest kanchanaravi1520/myweb:0.0.2' 
   }
}
}
